module github.com/solardev-xyz/freedom-node-mobile

go 1.26.2

// Inherited from Solar-Punk-Ltd/bee-lite-java. The sub-module
// `github.com/btcsuite/btcd/chaincfg/chainhash v1.0.1` shadows the same
// import path inside `github.com/btcsuite/btcd v0.22.0-beta`, which bee's
// transitive deps pull in. Without this exclude, `go mod tidy` errors with
// an ambiguous-import error.
exclude github.com/btcsuite/btcd/chaincfg/chainhash v1.0.1

// The old monolithic `google.golang.org/genproto` was split into
// per-API submodules (`googleapis/api`, `googleapis/rpc`, ...) in 2023.
// The `googleapis/api/*` and `googleapis/rpc/*` directories still exist
// inside the monolithic module tree though, so any build that has both
// the monolithic and the split submodules in its graph hits ambiguous-
// import errors at those paths.
//
// Our graph combines bee-lite (whose transitive deps still pin old
// cloud SDKs that require the monolithic module) with kubo (which
// targets only the split submodules). To eliminate the ambiguity we
// redirect the monolithic module to an empty stub (`stubs/genproto/`)
// so only the split submodules provide those paths. The only real
// consumer of the monolithic paths is test fixtures inside
// grpc-gateway's `runtime/internal/examplepb`, which we neither build
// nor test.
...
require (
	...
	github.com/ethersphere/batch-archive v0.0.5 // indirect
